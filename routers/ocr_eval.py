...
def evaluate_ocr(body: OCREvalIn, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Step 0.5: OCR 시작 - job 상태 업데이트 (current_step='ocr_eval', status='running')
        try:
            job.current_step = 'ocr_eval'
            job.status = 'running'
            db.commit()
            logger.info(f"Job 상태 업데이트: job_id={job_id}, current_step='ocr_eval', status='running'")

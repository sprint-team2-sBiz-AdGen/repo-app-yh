...
### Phase 1: 기본 로직 구현 (✅ 완료)
- [x] `_process_job_state_change` 함수에 뒤처진 variants 확인 로직 추가
- [x] 단계 순서 정의 (STEP_ORDER)
- [x] yh 파트 단계 확인 로직

### Phase 2: Variant 재시작 로직 (✅ 완료)
- [x] 뒤처진 variant 감지
- [x] done 상태 variant의 다음 단계 트리거
- [x] running/queued 상태 variant 처리
- [x] failed 상태 variant 재시도 로직

### Phase 3: 순환 참조 방지 (✅ 완료)
- [x] 재시작 전 상태 재확인 (중복 처리 방지)
- [x] 재시작 후 상태 확인 및 로깅 강화
- [x] 로깅 및 모니터링

### Phase 4: 테스트 (✅ 완료)
- [x] 뒤처진 variant 재시작 테스트
- [x] 실제 파이프라인에서 동작 확인
- [x] 복구 통계 로깅

### Phase 5: 수동 복구 로직 (✅ 완료)
- [x] `_periodic_recovery_check()` 백그라운드 태스크 추가
...
### 1. 뒤처진 Variants 자동 재시작 (✅ 완료)

**구현 위치**: `services/job_state_listener.py`

**주요 기능**:
- Job 상태 변경 시 뒤처진 variants 자동 감지
- `done` 상태 variant의 다음 단계 자동 트리거
- `failed` 상태 variant 재시도 로직
- `running` 상태 variant 감지 (5분 이상)
- 재시작 전/후 상태 재확인 (중복 처리 방지)
- 복구 통계 로깅 (`stuck_count`, `recovered_count`, `failed_count`)

**동작 방식**:
1. Job 상태가 `running` 또는 `failed`로 변경되고 yh 파트 단계인 경우
2. 해당 job의 모든 variants 조회
3. Job의 `current_step`보다 뒤처진 variants 찾기
4. 각 variant의 상태에 따라 재시작 또는 재시도
